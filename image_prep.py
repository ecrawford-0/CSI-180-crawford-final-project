...
def batch_rotate(folder_name,degrees):
    """
...
def show_choices_interface():
    ...
    while True:
        ...
        if user_choice == '3':
            current_dir = os.getcwd()
            folder_path = input("\nPlease enter the folder path:\n")
            rotate = input("Please enter the degrees to rotate the images by:")
            
            if not (os.path.isdir(folder_path)) or not(rotate.isnumeric() ) : # inputs are not valid
                print("Invalid inputs, make sure diretory exists and degrees to rotate by is a number")
           
            else: # inputs are valid
                batch_rotate(folder_path,int(rotate))
            os.chdir(start_dir)
        if user_choice =='4':
            return
